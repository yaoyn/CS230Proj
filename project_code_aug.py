...
# class_weight[0] = class_weight[0]*9
# class_weight[1] = class_weight[1]*3
# class_weight[5] = class_weight[5]*3
...
#9_3, 0.3
base_model = keras.applications.resnet.ResNet101(weights="imagenet", include_top=False, input_shape=(img_h, img_w, 3))
x = base_model.output
x = GlobalAveragePooling2D()(x)
x = Dropout(rate = 0.3)(x)
predictions = Dense(num_classes, activation= 'softmax')(x)
...
#9_3, 0.3
base_model = keras.applications.resnet.ResNet152(weights="imagenet", include_top=False, input_shape=(img_h, img_w, 3))
x = base_model.output
x = GlobalAveragePooling2D()(x)
x = Dropout(rate = 0.3)(x)
predictions = Dense(num_classes, activation= 'softmax')(x)
